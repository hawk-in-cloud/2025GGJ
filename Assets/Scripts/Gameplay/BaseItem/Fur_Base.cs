...
public class Fur_Base : MonoBehaviour
{
    ...
    //攻击所需要的子物体，子物体挂载一个在攻击时出现的collider2D
    ...
    private void Awake()
    {
        ...
        if (transform.Find("AttackCollider").gameObject == null)
        {
            Debug.Log("Can/t find AttackCollider");
        }
        else
        {
            attackColliderObj = transform.Find("AttackCollider").gameObject;
            _attackCollider = attackColliderObj.GetComponent<Collider2D>();
        }
